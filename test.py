...
def evaluate(model, path, iou_thres, conf_thres, nms_thres, img_size, batch_size):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=40, shuffle=False, num_workers=4, collate_fn=dataset.collate_fn
    )
    ...
    for batch_i, (_, imgs, targets) in enumerate(tqdm.tqdm(dataloader, desc="Detecting objects")):
        ...
        for inx, output in enumerate(outputs):
            ...
            center_preds.append(output[..., 7:9].data.cpu().numpy())
            coef_preds.append(output[..., 9:27].data.cpu().numpy())
            # print(coef_preds[0].shape)
